...
describe('gen entity', () => {
  ...
  it('generates Drizzle table + router + test when a Node backend uses Drizzle', async () => {
    ...
    expect(router).toContain("import { invoices } from '../../db/schema.js';");
    expect(router).toContain('registerEntityRoutes(');
    expect(router).toContain('table: invoices,');
    ...
    expect(appTs).toContain('registerInvoiceEntity(app, db);');
  ...
  it('generates Sequelize model + router + test when a Node backend uses Sequelize', async () => {
    dest = join(tmpdir(), `projx-gen-sequelize-${Date.now()}`);
    await scaffold(
      {
        name: 'gen-app',
        components: ['fastify'],
    ...
    expect(modelsIndex).toContain('Invoice,');

    const router = await readFile(
      join(dest, 'fastify/src/modules/invoice/index.ts'),
      'utf-8',
    );
    expect(router).toContain('model: Invoice,');

    const appTs = await readFile(join(dest, 'fastify/src/app.ts'), 'utf-8');
    expect(appTs).toContain(
      "import { registerInvoiceEntity } from './modules/invoice/index.js';",
    );
    expect(appTs).toContain('await registerInvoiceEntity(app);');

    expect(
      existsSync(join(dest, 'fastify/tests/modules/invoice.test.ts')),
    ).toBe(true);
  });
  ...
  it('generates TypeORM entity + router + test when a Node backend uses TypeORM', async () => {
    ...
    expect(entitySrc).toContain("@PrimaryGeneratedColumn('uuid')");
    expect(entitySrc).toContain("@Column({ type: 'varchar', name: 'name' })");
    expect(entitySrc).toContain("@Column({ type: 'integer', name: 'amount' })");
    ...
    expect(entitiesIndex).toContain('Invoice,');
