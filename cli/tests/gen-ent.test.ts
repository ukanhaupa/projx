...
describe('gen entity (go + ent) — SoftDeleteMixin sourcing', () => {
  ...
  it('ships SoftDeleteMixin in shared ent/schema/_mixins.go (not inside post.go)', async () => {
    ...
    const mixinsPath = join(dest, 'go/ent/schema/_mixins.go');
  ...
  it('generated entity schema lands in the same package as _mixins.go', async () => {
...
describe('gen entity (go + ent) — integration surface', () => {
  ...
  it('soft-delete sample schema includes SoftDeleteMixin{} and ships _mixins.go', async () => {
