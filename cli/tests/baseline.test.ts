...
import { matchesSkip, saveBaselineRef, applyTemplate, type GeneratorVars } from "../src/baseline.js";
...
describe("3-way merge", () => {
  ...
  it("preserves user additions when template changes different lines", async () => {
    ...
    const hasRef = (() => {
      try {
        execSync("git rev-parse --verify refs/projx/baseline", { cwd: dest, stdio: "pipe" });
        return true;
      } catch {
        return false;
      }
  ...
  it("baseline ref is updated after successful update", async () => {
    ...
    const refBefore = execSync("git rev-parse refs/projx/baseline", { cwd: dest, stdio: "pipe" }).toString().trim();
    await update(dest, REPO_DIR);
    const refAfter = execSync("git rev-parse refs/projx/baseline", { cwd: dest, stdio: "pipe" }).toString().trim();
...
describe("tier 1: git merge via worktree", () => {
  ...
  it("preserves user files not in template", async () => {
    ...
    expect(await readFile(join(dest, "fastify/src/my-service.ts"), "utf-8")).toContain("MyService");
    expect(await readFile(join(dest, "notes.txt"), "utf-8")).toContain("project notes");
  });
...
describe("tier 2: per-file 3-way merge", () => {
  ...
  it("user-created files survive 3-way merge", async () => {
    ...
    expect(await readFile(join(dest, "fastify/src/my-middleware.ts"), "utf-8")).toContain("mw");
  });
  ...
  it("multiple components — each file merges independently", async () => {
    ...
    await scaffold(
      { name: "t2-app", components: ["fastify", "e2e"], git: true, install: false },
      dest,
      REPO_DIR,
    );
    ...
    fpkg = fpkg.replace('"description":', '"fastify-custom": true,\n  "description":');
...
describe("cross-tier edge cases", () => {
  ...
  it("worktree cleanup on error does not corrupt repo", async () => {
    ...
    // No stale worktrees
    const worktrees = execSync("git worktree list", { cwd: dest, stdio: "pipe" }).toString();
