...
describe("runPrompts", () => {
  ...
  it("uses provided name and prompts for components and pm", async () => {
    vi.mocked(p.multiselect).mockResolvedValueOnce(["fastify", "frontend"] as never);
    vi.mocked(p.select).mockResolvedValueOnce("pnpm" as never);
  ...
  it("prompts for project name when not provided and validates input", async () => {
    ...
    vi.mocked(p.select).mockResolvedValueOnce("npm" as never);
  ...
  it("warns when no components are selected", async () => {
    ...
    expect(p.log.warn).toHaveBeenCalledWith(expect.stringContaining("No components"));
    expect(opts.components).toEqual([]);
  });

  it("skips package-manager prompt when no JS component selected", async () => {
    vi.mocked(p.multiselect).mockResolvedValueOnce(["fastapi", "infra"] as never);
  ...
  it("exits 0 when name prompt is cancelled", async () => {
    vi.mocked(p.text).mockResolvedValueOnce(Symbol.for("clack.cancel") as never);
  ...
  it("exits 0 when components prompt is cancelled", async () => {
    vi.mocked(p.multiselect).mockResolvedValueOnce(Symbol.for("clack.cancel") as never);
    const exitSpy = vi.spyOn(process, "exit").mockImplementation(() => {
      throw new Error("EXIT");
    });
  ...
  it("exits 0 when package-manager prompt is cancelled", async () => {
    vi.mocked(p.multiselect).mockResolvedValueOnce(["fastify"] as never);
    vi.mocked(p.select).mockResolvedValueOnce(Symbol.for("clack.cancel") as never);
    const exitSpy = vi.spyOn(process, "exit").mockImplementation(() => {
      throw new Error("EXIT");
    });
