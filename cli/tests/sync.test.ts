...
describe('sync helpers', () => {
  ...
  it('pickBackend prefers explicit override and validates presence', () => {
    expect(pickBackend(['fastify', 'frontend'])).toBe('fastify');
    expect(pickBackend(['go', 'frontend'])).toBe('go');
    expect(pickBackend(['go', 'frontend'], 'go')).toBe('go');
    expect(() => pickBackend(['fastify', 'frontend'], 'go')).toThrow(
      /not found in this project/,
    );
    expect(() => pickBackend(['frontend'])).toThrow(/No backend component/);
  });
...
describe('syncTypes', () => {
  ...
  it('writes types into frontend/src/types and updates the barrel', async () => {
    dest = join(tmpdir(), `projx-sync-${Date.now()}`);
    await scaffold(
      {
        name: 'sync-app',
        components: ['frontend', 'go'],
        git: true,
        install: false,
      },
      dest,
    ...
    const typeFile = await readFile(
      join(dest, 'frontend/src/types/widget.ts'),
      'utf-8',
    );
  ...
  it('merges with an existing barrel without duplicating lines', async () => {
    ...
    const typesDir = join(dest, 'frontend/src/types');
