...
- **pnpm is the default package manager** (`DEFAULT_PACKAGE_MANAGER` in `src/utils.ts`) for new JS scaffolds — `create`/`init` default to it; npm/yarn/bun stay selectable. On scaffold, JS components get `"packageManager": "pnpm@<PNPM_VERSION>"` stamped into `package.json` (`stampPackageManager` in `src/baseline.ts`, pnpm-only). The pinned version lives once as `pmCommands(pm).version` (`PNPM_VERSION`, currently `pnpm@11.5.2+sha512…`) and also renders the Dockerfile `corepack prepare` line — including the ORM-addon Dockerfile in `ormNodeDockerfileSource` (`pm.version`, not a literal) — so don't re-hardcode it. **pnpm-specific config lives in each template's `pnpm-workspace.yaml`** (`overrides:` for security pins, `allowBuilds:` for build-script approval), **not** the package.json `pnpm` field — pnpm 11 stopped reading that field, and a frozen install mismatches a lockfile that recorded the old `pnpm.overrides`. Back-compat: `add`/`update`/`gen`/`diff` still read the PM from the project's `.projx` (missing field → npm), so existing projects never flip.
- **Shipped artifacts never hard-code a component directory** (root §"Never hard-code a component directory in a shipped artifact"). Rendered `.ejs` use `<%= paths.<type> %>` / iterate `*Instances`; `resolvePrimarySourceDirs(paths)` in `src/utils.ts` is injected into every `renderEjsInDir` call as `backendSourceDir` / `frontendSourceDir` (for infra's single-pipeline / tfvars assumption). Static shell (`scripts/projx-dirs.sh`, `validate-nginx-config.sh`) is added to the `staticScripts` copy list in `src/utils.ts`. Covered by `tests/instance-aware-dirs.test.ts`.
