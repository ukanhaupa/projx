...
export async function syncTypes(
  ...
  const frontendComp = components.includes('vitejs')
    ? 'vitejs'
  ...
  if (!frontendComp) {
    throw new Error(
      `No frontend component found. sync writes types into <vitejs|nextjs>/src/types.`,
    );
  }
  ...
  const frontendDir = join(cwd, (paths as ComponentPaths)[frontendComp]);
