...
async function resolveRotationGraceChild(
  ...
  const child = await RefreshToken.findOne({ where: { id: token.rotated_to } });
  if (
    !child ||
...
export default fp(async (fastify) => {
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        await sendInitialVerificationEmail(fastify.sequelize, user.id);
      } catch (e) {
        fastify.log.error(
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (user.mfa_enabled) {
        ...
        const challenge_token = signMfaChallenge(fastify, user);
        return reply.send({
          mfa_required: true,
          challenge_token,
          email: user.email,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const revokeForReplay = async (replayTokenId: string) => {
        ...
        await fastify.sequelize.transaction(async (t) => {
          ...
          await RefreshToken.update(
            ...
            { where: { id: replayTokenId }, transaction: t },
          );
        });
        request.log.warn(
          {
      ...
      for (let attempt = 1; ; attempt++) {
        ...
        const newToken = await fastify.sequelize.transaction(async (t) => {
          ...
          await RefreshToken.update(
            ...
            { where: { id: claimedTokenId }, transaction: t },
          );
