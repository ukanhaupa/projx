...
async function recordMfaFailure(
  ...
  await fastify.db.update(users).set(data).where(eq(users.id, user.id));
}
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const totalUsers = Number(
        (await fastify.db.select({ c: count() }).from(users))[0].c,
      );
      ...
      try {
        await sendInitialVerificationEmail(fastify.db, user.id);
      } catch (e) {
        fastify.log.error(
