...
func (r *Repo) List(ctx context.Context, t *Table, offset, limit int) (*Page, error) {
	...
	order := ""
	if t.PrimaryKey != "" {
		order = " ORDER BY " + ident(t.PrimaryKey)
	}

	sql := fmt.Sprintf(
		"SELECT %s FROM %s%s LIMIT %d OFFSET %d",
		strings.Join(cols, ", "), qualified(t), order, limit+1, offset,
	)
	...
	page := &Page{Columns: names, Offset: offset, Limit: limit}
	for rows.Next() {
